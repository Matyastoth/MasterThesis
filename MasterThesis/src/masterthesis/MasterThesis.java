...
public class MasterThesis {
    ...
    public static void predict(File input, double t1, boolean guessR) throws IOException, Exception {
        //look for approximate resistance around this time point
        if (t1 == Double.NaN) {
            System.err.println("No time point given!");
            return;
        }
        if (guessR) {
        ...
        double[] T = new double[i];
        double[] visc = new double[i];
        double[] dens = new double[i];
        double[] TMP = new double[i];
        double[] R = new double[i];
        double[] integral = new double[i];
        double[] a = new double[i];
        double[] Rads = new double[i];
        double[] n = new double[i];
        double[] mcap = new double[i];

        double R0;
        double R1;

        //This part is hard-coded with column ids. Make sure it's right.
        String line;
        String[] tokens;
        i = 0;
        while ((line = br.readLine()) != null) {
            tokens = line.split(";");
            t[i] = Double.parseDouble(tokens[0]);
            J[i] = Double.parseDouble(tokens[1]);
            c[i] = Double.parseDouble(tokens[3]);
            T[i] = Double.parseDouble(tokens[4]);
            visc[i] = Double.parseDouble(tokens[9]);
            dens[i] = Double.parseDouble(tokens[8]);
            TMP[i] = Double.parseDouble(tokens[10]);
            R[i] = Double.parseDouble(tokens[11]);
            i++;
        }
        
        i = 0;
        while (i < t.length) {
            a[i] = 1 - Math.pow(Math.pow(R[0], 2) * dens[i] * TMP[i] * Math.pow(visc[0], 2)/ (Math.pow(R[i], 2) * dens[0] * TMP[0] * Math.pow(visc[i], 2)),0.4);
            //a[i] = 1-Math.pow(Math.pow(R[0],1) / Math.pow(R[i],1), 0.4);
            i++;
        }
        
        File script = new File("C:\\\\Users\\\\Matt\\\\university\\\\Aalborg\\\\master\\\\gnuplot\\\\script1.txt");
        File data1 = new File(script.getParentFile().getAbsolutePath() + "\\data.txt");

        BufferedWriter bw = new BufferedWriter(new FileWriter(data1));
        i = 0;

        double alpha = 0.35;
        double cutoff = 0.05;
        //double[] a_av = UtilsForGeneralUse.smoothExponential(a[1], a, alpha, cutoff);
        double[] a_av = UtilsForGeneralUse.smoothExponential(a[1], a, 0.30, cutoff);

        int aver = 5;
        //Approximately how much early data must be taken into account.
        double time_av = 5;
        double Rf;
        double[] Rf_a = new double[t.length];
        double ax;
        double[] ax_a = new double[t.length];
        double tx;
        double[] tx_a = new double[t.length];
        double Rx = 5.4*Math.pow(10, 9);
        while (i < t.length) {

            if (aver < 1) {
                //UtilsForGeneralUse.println(bw, t[i] + DS + a_av[i]);
            } else if (i - aver > 0) {
                //UtilsForGeneralUse.println(bw, t[i] + DS + (a[i]-a[i-aver])/(t[i]-t[i-aver]));
            }
            /*
             if (i - aver > 0) {
             Rf = (a_av[i] - a_av[i - aver]) / (t[i] - t[i - aver]);
             Rf_a[i] = Rf;
             //ax = 1 - Math.pow(Math.pow(R0, 2) * dens[i] * TMP[i] * Math.pow(visc0, 2) / (Math.pow(Rx, 2) * dens0 * TMP0 * Math.pow(visc[i], 2)), 0.4);
             ax = 1 - (1 - a[i]) * Math.pow(Math.pow(R[i], 2) / (Math.pow(Rx, 2)), 0.4);
             ax_a[i] = ax;
             tx = t[i] * (ax - a_av[i]) / Rf;
             tx_a[i] = tx;
             if (R[i] < Rx) {
             //UtilsForGeneralUse.println(bw, t[i] + DS + (t[i] + tx));
             }
             }
             */
            ...
            try {
                //Lets make a test!
                //closestIndex = UtilsForGeneralUse.getClosestIndex(t, i, time_av, -1);
                closestIndex = 0;
            }
            ...
            if (closestIndex == Integer.MIN_VALUE) {
                i++;
                continue;
            }
            //System.out.println((a[i] - a[closestIndex]));
            Rf = (a[i] - a[closestIndex]) / (t[i] - t[closestIndex]);
            Rf_a[i] = Rf;
            ax = 1 - (1 - a[i]) * Math.pow(Math.pow(R[i], 2) / (Math.pow(Rx, 2)), 0.4);
            ax_a[i] = ax;
            //tx = t[i] + (ax - a[i]) / Rf;
            tx = t[i] + Math.pow((Rx-R[i])*(t[i]-t[0])/(R[i]-R[0]), 1);
            tx_a[i] = tx;
        ...
        double[] tx_av = new double[t.length];
        while (i < t.length) {
            tx_av = UtilsForGeneralUse.smoothExponential(0, tx_a, 0.99, 100);
            //UtilsForGeneralUse.println(bw, t[i] + DS + (t[i] + tx_av[i]));
            i++;
        }

        i = 0;
        double[] Rf_av = UtilsForGeneralUse.smoothExponential(0, Rf_a, 0.40, 0.3);
        double[] ax_av = UtilsForGeneralUse.smoothExponential(0, ax_a, 0.40, 0.5);
        while (i < t.length) {
            //tx_a[i] = t[i] + (ax_av[i] - a_av[i]) / Rf_av[i];

            tx_av = UtilsForGeneralUse.smoothExponential(1, tx_a, 0.6, 0.5);

            if (R[i] < Rx) {
                UtilsForGeneralUse.println(bw, t[i] + DS + tx_a[i]);
                //UtilsForGeneralUse.println(bw, t[i]/t1*100 + DS + 100*Math.abs(tx_a[i]-t1)/t1);
                //UtilsForGeneralUse.println(bw, t[i]/t1*100 + DS + (tx_a[i]-t[i])/t[i]);
            }
            i++;
        }
        bw.close();
        Runtime.getRuntime().exec("cmd /k start \"\" \"c:/Program Files (x86)/gnuplot/bin/gnuplot.exe\" " + script.getAbsolutePath());
        //Runtime.getRuntime().exec("cmd /k start \"\" \"c:/Program Files (x86)/gnuplot/bin/gnuplot.exe\"");
    }
